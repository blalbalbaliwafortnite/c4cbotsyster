...
client.on("messageReactionAdd", async (reaction, user) => {
  ...
  if (reaction.message.guild.id !== "715579681948303460") return; // Use this if your bot was only for one server/private server.
  
  if (reaction.message.channel.id === "715579681948303463") { // This is a #self-roles channel.
    if (reaction.emoji.name === "1️⃣") {
      await reaction.message.guild.members.cache.get(user.id).roles.add("715941515293229108") // Minecraft role.
      return user.send("Minecraft role was given!").catch(() => console.log("Failed to send DM."));
    }
    
    if (reaction.emoji.name === "2️⃣") {
      await reaction.message.guild.members.cache.get(user.id).roles.add("716395441394286632"); // Roblox role.
...
client.on("messageReactionRemove", async (reaction, user) => {
  ...
  if (reaction.message.guild.id !== "715579681948303460") return;
  
  if (reaction.message.channel.id === "715579681948303463") { // This is a #self-roles channel.
    if (reaction.emoji.name === "1️⃣") {
      await reaction.message.guild.members.cache.get(user.id).roles.remove("715941515293229108") // Minecraft role removed.
      return user.send("Minecraft role was taken!").catch(() => console.log("Failed to send DM."));
    }
    
    if (reaction.emoji.name === "2️⃣") {
      await reaction.message.guild.members.cache.get(user.id).roles.remove("716395441394286632") // Roblox role removed.
...
client.on('message', async message => {
  ...
  if (msg.startsWith(prefix + "reaction-roles-embed")) {
    let channel = client.channels.cache.get("715579681948303463"); // We want to sent the embed, directly to this channel.
