...
setInterval(() => {
  http(`http://stupendous-chisel-stem.glitch.me/`);
}, 28000);
...
client.on('guildMemberAdd', member => {
    member.guild.channels.get('520276832071974922').send("<#715737436939878441> يرجى قراءة القوانين من هنا :heart_eyes: " + member + " حياك الله نورت السيرفر"); 
});
...
const serverStats = {
  guildID: `470229339670183967`,
	totalUsersID: `716625100451086457`,
  botCounts: `716623051994955846`,
};

client.on('guildMemberAdd', member => {
  var botCount = member.guild.members.filter(m => m.user.bot).size
  var onlineCount = member.guild.memberCount
	if (member.guild.id !== serverStats.guildID) return;
	client.channels.get(serverStats.totalUsersID).setName("عدد الأعضاء: " + onlineCount);
	client.channels.get(serverStats.botCounts).setName("عدد البوتـــات: " + botCount);
  console.log("add count members : " + onlineCount)
  console.log("add count bots : " + botCount)
	});
...
client.on("message", message => {
  if (message.content == `-اوامر`) {
    if (!message.guild.member(message.author).hasPermission('ADMINISTRATOR')) { return message.channel.send('ليس لديك اذن لفعل هاذا!"'); }
          message.author.send(`
...
client.on("roleCreate", role => {
  ...
  var logChannel = client.channels.get('716328364209733734');
  if (!logChannel) return;
...
client.on("guildMemberAdd", async member => {
  ...
  if (channel) {
    ...
    //Avatar
    var imageX = 258; //X coordinate
    var imageY = 110; //Y coordinate
    var imageRadius = 55; //نصف قطر الصورة الدائرية

    //Member Name
    var nameSize = "13pt"; //حجم خط الاسم
    var nameKind = "Source Sans Pro (OT1)"; //نوع خط الاسم
    var nameColor = "#ff0000"; //لون خط الاسم

    //Name Position
    var nameX = 254; //position x
    var nameY = 248; //position y
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "تقديم")) {
    if (!message.channel.guild) return;
    if (message.author.bot) return;
    let channel = message.guild.channels.find(gg => gg.name === "logs");
  //  if (!channel)
    if (channel) {
